...
<form method="GET" action="{{ route('ingredientes.index') }}" class="row g-2 mb-4">
    <div class="col-md-4">
    ...
    <div class="col-auto">
        ...
        <a href="{{ route('ingredientes.index') }}" class="btn btn-outline-secondary ms-1">Limpiar</a>
    </div>
</form>
...
<div class="card shadow-sm">
    <div class="card-body p-0">
        <table class="table table-hover mb-0">
            ...
            <tbody>
            @forelse($ingredientes as $i)
                <tr>
                    <td>{{ $i->nombre }}</td>
                    <td>{{ $i->tienda->nombre_tienda ?? '—' }}</td>
                    <td class="text-end">
                        <a href="{{ route('ingredientes.edit', $i) }}" class="btn btn-sm btn-outline-primary me-1">
                            <i class="bi bi-pencil"></i>
                        </a>
                        <form action="{{ route('ingredientes.destroy', $i) }}" method="POST" class="d-inline"
